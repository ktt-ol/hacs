...
class OverviewFragment : BasePageFragment(), NetworkStatusListener {
    ...
    override fun onCreateView(
        ...
        view.findViewById<View>(R.id.overview_buzzer_outer).setOnClickListener {
            interaction.sendSshCommand(SPACE_DOOR, DoorCommand.getOuterDoorBuzzerCmd())
        }
        view.findViewById<View>(R.id.overview_buzzer_inner_glass).setOnClickListener {
            interaction.sendSshCommand(SPACE_DOOR, DoorCommand.getInnerGlassDoorBuzzerCmd())
        }
        view.findViewById<View>(R.id.overview_buzzer_inner_metal).setOnClickListener {
            interaction.sendSshCommand(SPACE_DOOR, DoorCommand.getInnerMetalDoorBuzzerCmd())
        }
        ...
        val trashInfo = interaction.trashCalendar.trashSummaryForTomorrow
        if (trashInfo == null) {
    ...
    override fun getTitleRes(): Int = R.string.nav_overview

    private fun setStatusText(status: Status?) {
        val text = view?.findViewById<View>(R.id.overview_status) as? TextView
            ?: return
        text.text = if (status == null) getString(R.string.unknown) else status.uiValue
    }

    private fun setKeyholderText(keyholderText: String?) {
        val overview = (view?.findViewById<View>(R.id.overview_keyholder) as? TextView) ?: return
        overview.text = when {
    ...
    private fun setDevicesText(devices: SpaceDevices?) {
        ...
        if (devices != null) {
            ...
            buffer.append("<br>\nOrte: ")
            for (colorPair in locationColor.all) {
